...
void Image::init(const uint16_t* buffer, ColorMode col) {
  ...
  _width = *(buf++);
  _height = *(buf++);
  frame_looping = DEFAULT_FRAME_LOOP;
  transparentColor = 0xF81F;
  colorMode = col;
...
Image::Image(const uint16_t* buffer, ColorMode col, uint16_t _frames) : Graphics(0, 0) {
  freshStart();
  init(buffer, col, _frames);
}

void Image::init(const uint16_t* buffer, ColorMode col, uint16_t _frames) {
  if (isObjectCopy) {
    return;
  }
  if (frame_handler) {
