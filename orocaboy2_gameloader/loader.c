...
uint8_t loader_cmd_buffer[16*1024];
...
bool loaderInit(uint8_t channel, char *port_name, uint32_t baud)
{
  ...
  if (cmdBegin(&cmd_loader, channel, port_name, baud) == false)
  {
    return false;
  }
...
err_code_t loaderCmdReadVersion(uint8_t *p_version)
{
  ...
  cmd_t *p_cmd = &cmd_loader;
  uint32_t i;


  errcode = cmdSendCmdRxResp(p_cmd, LOADER_CMD_READ_VERSION, NULL, 0, 1000);
  if (errcode == OK)
  {
    for (i=0; i<2; i++)
    {
...
err_code_t loaderCmdReadPacketDataSizeMax(uint16_t *p_size)
{
  ...
  errcode = cmdSendCmdRxResp(p_cmd, LOADER_CMD_PACKET_DATA_SIZE_MAX, NULL, 0, 1000);
  if (errcode == OK)
  {
...
err_code_t loaderCmdReadBoardName(uint8_t *p_str)
{
  ...
  errcode = cmdSendCmdRxResp(p_cmd, LOADER_CMD_READ_BOARD_NAME, NULL, 0, 1000);
  if (errcode == OK)
  {
...
err_code_t loaderCmdFlashErase(uint32_t addr, uint32_t length)
{
  ...
  errcode = cmdSendCmdRxResp(p_cmd, LOADER_CMD_FLASH_ERASE, data, index, 10000);
...
err_code_t loaderCmdFlashWrite(uint32_t addr, uint8_t *p_data, uint32_t length)
{
  ...
  cmd_t *p_cmd = &cmd_loader;
  uint32_t i;
  uint8_t *data = loader_cmd_buffer;
  uint32_t index = 0;
  ...
  errcode = cmdSendCmdRxResp(p_cmd, LOADER_CMD_FLASH_WRITE, data, index, 1000);
...
err_code_t loaderCmdFlashVerfy(uint32_t addr, uint32_t length, uint16_t tx_crc, uint16_t *rx_crc)
{
  ...
  errcode = cmdSendCmdRxResp(p_cmd, LOADER_CMD_FLASH_VERIFY, data, index, 3000);
...
err_code_t loaderCmdJumpToAddress(uint32_t addr)
{
  err_code_t errcode = OK;
  cmd_t *p_cmd = &cmd_loader;
  uint8_t *data = loader_cmd_buffer;
  ...
  errcode = cmdSendCmdRxResp(p_cmd, LOADER_CMD_JUMP, data, index, 1000);
